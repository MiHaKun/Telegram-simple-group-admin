...
from telegram.ext import ApplicationBuilder, ContextTypes, CommandHandler, MessageHandler, CallbackQueryHandler, filters, PicklePersistence, ChatJoinRequestHandler
...
# 延时ban掉用户
async def ban_user_later(delay: float, chat_id, user_id: int, context: ContextTypes.DEFAULT_TYPE):
    name=f"banjob_{chat_id}_{user_id}"
    context.job_queue.run_once(_ban_user_cb, delay, chat_id=chat_id, name=name , data=f"{user_id}-0")
    return name
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id in config['admin_ids']:
        ...
        await update.message.reply_html(f"本群机器人并不支持普通用户使用。\n\n请联系管理员 @{config['contact_username']} 获取帮助。")

# 利用deeplink区分不同的跳转来源。
# 这个函数响了，代表是用户过来通过验证。
# 用户私聊机器人，走的上面的 start 函数。
async def start_with_deep_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info(f"User {update.effective_user.id} start with deep link. {update.message.text}")
    user = update.effective_user
    prefix, user_id, chat_id = update.message.text.split('_')
    if prefix != '/start joingroup' or user_id != str(user.id) or chat_id != str(config['group_id']):
    ...
    sent = await context.bot.send_photo(user.id, 
                                        photo, 
                                        f"{mention_html(user.id, user.first_name)}请选择图片中的文字。回答错误将入群。", reply_markup=InlineKeyboardMarkup(button_matrix),
                                        parse_mode="HTML")
    ...
    await delete_message_later(config['ban_after'], sent.chat.id, sent.message_id, context)       
...
async def join_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.chat_join_request:
        return
    logger.info(f"New user {update.chat_join_request.from_user.id} request to join group {update.effective_chat.id}")
    ...
    # 群内提醒用户
    button = InlineKeyboardButton("点击加入", url=create_deep_linked_url(context.bot.username, f"joingroup_{user.id}_{chat.id}"))
    sent = await context.bot.send_message(
        chat.id, 
        config['msg_new_user_joined_group'].format(mention_html(user.id, user.full_name)) , 
        parse_mode="HTML", 
...
async def callback_query_vcode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == str(user.id):
        # 是正确的人点击
        logger.info(f"User {user.id} clicked {code}, the right code is {context.user_data.get('vcode')}")
        if code == context.user_data.get('vcode'):
            ...
            await context.bot.restrict_chat_member(chat.id, user.id, chat.permissions, 0)            
            ...
            await context.bot.send_message(user.id, f"你的验证码错误，等待20分钟后再次尝试入群。")
...
if __name__ == '__main__':
    pickle_persistence = PicklePersistence(filepath=f"./assets/{config['app_name']}.pickle")
    application = ApplicationBuilder().token(config['bot_token']).persistence(persistence=pickle_persistence).build()
    ...
    application.add_handler(CommandHandler('start', start_with_deep_link, filters.Regex(r'joingroup')))
    application.add_handler(CommandHandler('start', start, filters.ChatType.PRIVATE & ~filters.Regex(r'joingroup')))
    ...
    logger.info(f"Bot @{application.bot.username} Started.")
